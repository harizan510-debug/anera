...
const HANDLES = [
  { key: 'instagram_handle', label: 'IG', baseUrl: 'https://instagram.com/' },
  { key: 'tiktok_handle',   label: 'TT', baseUrl: 'https://tiktok.com/@' },
  { key: 'facebook_handle', label: 'FB', baseUrl: 'https://facebook.com/' },
] as const;
...
export default function Social() {
  ...
  return (
    <div className="pb-4" style={{ background: '#F8F7F4', minHeight: '100%' }}>
      {/* Header */}
      ...
      {loading ? (
        ...
        <div className="pt-4" style={{ background: '#F8F7F4' }}>
          {posts.map(post => (
    ...
    <div className="mx-4 mb-4 rounded-2xl overflow-hidden" style={{ background: '#FFFFFF', boxShadow: '0 4px 20px rgba(0,0,0,0.05)' }}>
      ...
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {post.occasion && <span className="text-xs font-medium px-2 py-0.5 rounded-full inline-block mt-0.5" style={{ background: '#F0EDE8', color: '#6F6F6F' }}>{post.occasion}</span>}
        </div>
      ...
      <div className="px-4 py-3">
        <div className="flex items-center gap-4 mb-2">
          <button onClick={onLike} className="flex items-center gap-1.5 transition-transform active:scale-90">
            <Heart size={22} fill={post.user_has_liked ? '#A3B18A' : 'none'}
              stroke={post.user_has_liked ? '#A3B18A' : 'var(--text-primary)'} strokeWidth={1.8} />
        ...
        {post.tags?.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-1.5">
            {post.tags.map((tag, i) => <span key={tag} className="text-xs font-medium px-2 py-0.5 rounded-full" style={{ background: i % 2 === 0 ? '#DDE5D3' : '#F0EDE8', color: '#6F6F6F' }}>#{tag}</span>)}
          </div>
        )}
        ...
        <div className="flex gap-2 mb-4">
          {(['image', 'video'] as const).map(type => (
            <button key={type} onClick={() => { setAcceptType(type === 'image' ? 'image/*' : 'video/*'); }}
              ...
              style={{
                background: mediaType === type ? '#A3B18A' : 'var(--bg)',
                color: mediaType === type ? '#1A1A1A' : 'var(--text-secondary)',
                border: `1px solid ${mediaType === type ? '#A3B18A' : 'var(--border)'}`,
              }}>
      ...
      <div className="w-full rounded-t-3xl" style={{ background: 'var(--surface)', maxHeight: '75vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div className="flex gap-3 px-4 py-3 flex-shrink-0" style={{ borderTop: '1px solid var(--border)' }}>
          ...
          <button onClick={addComment} disabled={!newComment.trim() || loading}
            ...
            {loading ? <Loader2 size={15} color="#1A1A1A" className="animate-spin" /> : <Send size={15} color="#1A1A1A" />}
          </button>
        </div>
      </div>
    </div>
